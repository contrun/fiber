...
impl CchActor {
    ...
    async fn handle_pending_received_tlc_notification(
        ...
        log::debug!("[inbounding tlc] payment hash: {}", payment_hash);
        ...
        log::debug!("[inbounding tlc] SendPaymentRequest: {:?}", req);
        ...
        select! {
            payment_result_opt = stream.next() => {
                log::debug!("[inbounding tlc] payment result: {:?}", payment_result_opt);
