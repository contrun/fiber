...
impl<S: GossipMessageStore> ExtendedGossipMessageStoreState<S> {
    ...
    fn get_node_announcement(
        ...
        self.store
            ...
            .or_else(|| {
                (!only_in_store)
                    .then(|| self.get_node_announcement_in_memory(node_id))
                    .flatten()
            })
    }
    ...
    fn get_channel_annnouncement(
        ...
        self.store
            ...
            .or_else(|| {
                (!only_in_store)
                    .then(|| self.get_channel_annnouncement_in_memory(outpoint))
                    .flatten()
            })
    }
    ...
    fn get_dependent_messages_in_memory(
        ...
        match message {
            ...
            BroadcastMessageWithTimestamp::ChannelUpdate(channel_update) => {
                match self.get_channel_annnouncement(&channel_update.channel_outpoint, false) {
                    Some((timestamp, channel_announcement)) => {
        ...
        match message {
            ...
            ExtendedGossipMessageStoreMessage::SaveAndBroadcastMessage(message) => {
                match state.insert_message_to_be_saved_list(&message).await {
                    ...
                    Ok(message) => {
                        if state.has_transitive_dependencies(&message, true) {
                            let _ = state.gossip_actor.send_message(
                                GossipActorMessage::BroadcastMessageImmediately(message),
                            );
                        } else {
                            warn!(
                                "Message not broadcasted because of missing dependencies: {:?}",
                                message
                            );
                        }
                    }
